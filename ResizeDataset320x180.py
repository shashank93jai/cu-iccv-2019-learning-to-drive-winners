...
input_folder = './Drive360Images/Drive360Images/'
output_folder = './Drive360Images_320_180/'

files = pd.concat([pd.read_csv('Drive360Images/drive360challenge_test.csv'),
                   pd.read_csv('Drive360Images/drive360challenge_train.csv'),
                   pd.read_csv('Drive360Images/drive360challenge_validation.csv')])
cols = 4
...
for i in range(len(files)):
    ...
    for j in range(cols):
        file_in = input_folder + files.iloc[i,j]
        if file_in.endswith('jpg'):
            file_out = output_folder + files.iloc[i,j]
